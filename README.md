...
## <b>[06월 01일] </b> <a id="0601"></a>
### 1. 익스플로러에서는 사용할 수 없는 코드
자바스크립트 코드를 짤 때, Internet Explorer에서는 사용할 수 없는 코드들이 있다.
<br>1. let 키워드와 const 키워드
<br>2. 템플릿 문자열
<br>3. 화살표 함수
<br>4. for of 반복문
...
> prompt(<메시지>,<임시글자>) :  프롬프트를 출력합니다. <br> 
...
> - 메소드 <br>
> assign(<링크>) : 매개변수로 전달한 위치로 이동합니다. <br>
> reload() : 새로고침 합니다. <br>
...
### 3. 문서 객체 모델
문서모델 객체란? <br>
<B>넓은 의미</B> : 웹 브라우저가 HTML 페이지를 인식하는 방법 <br>
<B>좁은 의미</B> : document 객체와 관련된 객체의 집합을 나타냄 <br> 
즉, <B>HTML 태그를 자바스크립트에서 사용할 수 있는 객체</B>로 만드는 것이다. 

<br> 정적으로 문서 객체를 생성한다는 것은 웹 페이지를 실행할 때, HTML 페이지에 있는 태그를 읽으면서 생성하는 것을 의미하고.
<br> 동적으로 문서 객체를 생성한다는 것은 자바 스크립트를 사용해 프로그램 실행 중에 문서 객체를 실행하는 것이다.
...
> - 메소드 (여러개 선택) <br>
> document.getElementByName(이름) : name 속성으로 여러 개의 문서 객체를 선택합니다. <br>
> document.getElementByClassName(클래스) : class 속성으로 여러 개의 문서 객체를 선택합니다. <br>
> document.querySelectorAll(선택자) : 선택자로 여러 개의 문서 객체를 선택합니다. <br>
### 3-2. 문서 객체 조작
1. 문자조작
innerHTML 이라는 속성을 이용해서 태그 내에 있는 문자를 조작할 수 있다. 
2. 스타일 조작
background-color,
bordor-radius,
border-bottom 과 같은 속성으로 문서 객체로 스타일을 조작할 수 있다.
3. 속성 조작
> - 메소드 <br>
> setAttribute(속성이름, 속성값) : 속성을 지정합니다. <br>
> getAttribute(속성이름) : 속성값을 추출합니다. <br>

해당 메소드로 태그가 가지고 있는 width 값이나 color 값을 가져올 수 있습니다.

### 4. 이벤트
이벤트란 키보드로 키를 입력하거나, 마우스로 클릭하는 등 어떤 현상이 프로그램에 영향을 끼치는 것을 말합니다. 웹에는 여러가지 이벤트가 존재합니다.
1. 인라인 이벤트
인라인 이벤트는 HTML 태그 내부에서 이벤트를 연결하는 방법이다. 
```
<button onclick="alert('click')">버튼</button>
```
이런식으로 HTML 태그 안에 onclick 이벤트 속성을 부여하여 버튼이 클릭되었을 때 alert 함수가 실행되게끔 할 수 있다.
2. 고전 이벤트
고전 이벤트 모델은 자바스크립트 코드 내에서 이벤트를 지정하여 해당 함수를 실행시키는 형태이다.
```
<script>
...
이런식으로 스크립트 내부에 이벤트 속성을 정의하여 코드를 짤 수 있다.

### 6. jQuery
jQuery 라이브러리는 $ 함수를 활용한다.  
```
$(<매개 변수>).메소드(<매개변수>,<매개변수>)
```
